...
function computeOrderedNumbers(blocks: Block[]): number[] {
  ...
  for (const b of blocks) {
    if (b.type === 'ordered') { count++; nums.push(count) }
    else { count = 0; nums.push(0) }
  }
  return nums
}
...
const NoteEditor = forwardRef<NoteEditorHandle, Props>(
  ({ content, noteHeight, onChange, onFocusedTypeChange }, ref) => {
    ...
    const blocksRef = useRef(blocks)      // 항상 최신 blocks를 이벤트 핸들러에서 참조
    ...
    useImperativeHandle(ref, () => ({
      ...
      getFocusedType: () => blocksRef.current[focusedIdx.current]?.type ?? 'text',
    }))

    const handleChange = (idx: number, value: string) => {
      update(blocksRef.current.map((b, i) => i === idx ? { ...b, content: value } : b))
    }

    const handleToggle = (idx: number) => {
      update(blocksRef.current.map((b, i) => i === idx ? { ...b, checked: !b.checked } : b))
    }
    ...
    const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>, idx: number) => {
      ...
      if (e.key === 'Enter') {
        ...
        const newBlock: Block = { id: uid(), type: cur[idx].type, content: after, checked: false }
        const next = [...cur]
      ...
      if (e.key === 'ArrowUp' && idx > 0) {
        ...
        setTimeout(() => {
          ...
          if (prev) { prev.focus(); const pos = Math.min(col, prev.value.length); prev.setSelectionRange(pos, pos) }
        }, 0)
      }
      ...
      if (e.key === 'ArrowDown' && idx < cur.length - 1) {
        ...
        setTimeout(() => {
          ...
          if (next) { next.focus(); const pos = Math.min(col, next.value.length); next.setSelectionRange(pos, pos) }
        }, 0)
      }
      ...
      if (e.key === 'Backspace') {
        ...
        if (domValue === '' && cur.length > 1) {
          ...
          setTimeout(() => {
            const input = inputRefs.current[idx - 1]
            if (input) { input.focus(); input.setSelectionRange(prevContent.length, prevContent.length) }
          }, 0)
        }
      }
    }
    ...
    return (
      <div className="overflow-y-auto overflow-x-hidden" style={{ height: noteHeight - 44 }}>
        <div className="p-3 space-y-0.5">
          {blocks.map((block, idx) => (
            <div key={block.id} className="flex items-center gap-2 group min-h-[26px]">
              ...
              {block.type === 'check' && (
                ...
                >
                  {block.checked && (
                    <svg viewBox="0 0 10 10" className="w-full h-full fill-none stroke-white" strokeWidth="2.2">
                      <polyline points="2,5 4,7.5 8,3" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  )}
                </button>
              )}
              ...
              <input
                ...
                placeholder={idx === 0 && blocks.length === 1 ? '내용을 입력하세요...' : ''}
